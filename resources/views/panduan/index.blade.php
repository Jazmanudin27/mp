...
<div class="row">
    ...
    <div class="col-lg-8 col-md-12 col-sm-12">
            ...
            <div class="card-body p-4 text-center">
                ...
                <form action="{{ route('panduan.index') }}" method="GET" class="max-w-600 mx-auto">
                    <div class="input-group input-group-merge shadow-sm" style="border-radius: 50px; overflow: hidden;">
                        <span class="input-group-text bg-white border-0" id="basic-addon-search31"><i
                                class="ti ti-search text-muted fs-4"></i></span>
                        <input type="text" class="form-control border-0 bg-white" name="search"
                            value="{{ $search }}" placeholder="Ketik kata kunci (misal: BPB, Pembelian, dll)..."
                            aria-label="Search" aria-describedby="basic-addon-search31" style="height: 46px;">
                        @if ($search)
                            <a href="{{ route('panduan.index') }}"
                                class="btn btn-light bg-white border-0 text-muted d-flex align-items-center justify-content-center px-3"
                                title="Clear Search"><i class="ti ti-x"></i></a>
                        @endif
                    </div>
                </form>
            </div>
        ...
        @if ($kategoriGroup->isEmpty())
            <div class="card shadow-sm border text-center p-5 mb-4">
                ...
                <p class="text-muted small">Kata kunci <b>"{{ $search }}"</b> tidak cocok dengan artikel apa pun.
                    Coba kata kunci lainnya atau gunakan Q&A Chat.</p>
                <a href="{{ route('panduan.index') }}" class="btn btn-primary btn-sm mx-auto"
                    style="width: fit-content;">Lihat Semua Panduan</a>
            </div>
        @else
            <!-- Categories Grid -->
            @foreach ($kategoriGroup as $kategori => $artikels)
                <div class="card shadow-sm border mb-4">
                    <div class="card-header py-3 d-flex align-items-center gap-2"
                        style="background-color: #fafbfc; border-bottom: 1px solid #e2e8f0;">
                        <h6 class="m-0 fw-bold text-dark d-flex align-items-center gap-2">
                            <span class="fs-4">📂</span>
                            <span>Kategori: {{ $kategori }}</span>
                        </h6>
                        <span class="badge bg-secondary text-white rounded-pill ms-auto">{{ $artikels->count() }}
                            Panduan</span>
                    </div>
                    <div class="card-body p-0">
                        <div class="list-group list-group-flush">
                            @foreach ($artikels as $artikel)
                                <a href="{{ route('panduan.show', $artikel->slug) }}"
                                    class="list-group-item list-group-item-action d-flex align-items-center gap-3 py-3 border-0 border-bottom">
                                    <div class="bg-primary-subtle text-primary rounded-circle d-flex align-items-center justify-content-center"
                                        style="width: 42px; height: 42px; flex-shrink: 0; background-color: #eff6ff;">
    ...
    <div class="col-lg-4 col-md-12 col-sm-12">
                    ...
                    @foreach ($faqs as $index => $faq)
                        <div class="accordion-item border mb-2" style="border-radius: 8px; overflow: hidden;">
                            <h2 class="accordion-header" id="heading-{{ $index }}">
                                <button class="accordion-button collapsed py-2_5 px-3 fw-semibold small text-dark"
                                    type="button" data-bs-toggle="collapse"
                                    data-bs-target="#collapse-{{ $index }}" aria-expanded="false"
                                    aria-controls="collapse-{{ $index }}" style="font-size: 13px;">
                                    {{ $faq->pertanyaan }}
                                </button>
                            </h2>
                            <div id="collapse-{{ $index }}" class="accordion-collapse collapse"
                                ...
                                <div class="accordion-body p-3 bg-light text-muted small"
                                    style="line-height: 1.6; border-top: 1px solid #e2e8f0;">
                                    {!! $faq->jawaban !!}
                                </div>
                            </div>
                        </div>
        ...
        <div class="card shadow-sm border text-center p-4" style="background-color: #f8fafc;">
            ...
            <p class="text-muted small mb-3">Gunakan <b>Q&A Assistant Chat</b> di pojok kanan bawah halaman untuk
                bertanya langsung kepada bot asisten kami secara interaktif.</p>
            <button class="btn btn-primary btn-sm mx-auto d-flex align-items-center gap-1"
